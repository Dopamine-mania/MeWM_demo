...
except ImportError:  # MONAI newer versions
...
class ImageDataset(Dataset):
    def __init__(self, args, mode='train'):
        ...
        if mode == 'train':
            with open(self.args.data_list_file, 'r') as f:
                self.data_lines = f.readlines()
            # self.data_lines = self.data_lines[:int(len(self.data_lines) * 0.8)]
            self.transform = self.get_train_transform()
        elif mode == 'valid':
            with open(self.args.data_list_file_val, 'r') as f:
                self.data_lines = f.readlines()
    ...
    def __getitem__(self, idx):
        ...
        items = line.split('\t')
        patient_id = items[0].split('/')[1] 

        data_dict = {
            'image.pre': os.path.join(self.args.data_root, items[0]),
            'image.post': os.path.join(self.args.data_root, items[4]),
            'label.pre': os.path.join(self.args.data_root, items[1]),
            'label.post': os.path.join(self.args.data_root, items[5]),
        ...
        if self.mode == 'train':
            ...
            return {
                'input_CT_pre': data_dict['image.pre'],
                'input_CT_post': data_dict['image.post'],
                'label_CT_pre': data_dict['label.pre'],
                'label_CT_post': data_dict['label.post'],
                'survival_time': torch.tensor(data_dict['survival_time'], dtype=torch.float32),
                'event_indicator': torch.tensor(data_dict['event_indicator'], dtype=torch.float32),
                'patient_id': patient_id
            }
    ...
    def get_train_transform(self):
        return transforms.Compose([
            ...
            # CheckShapes(keys=["image.pre", "label.pre"], stage='train'),
            # Printd(keys=["image.pre"], print_shape=True),
            SpatialPadd(
                keys=["image.pre", "image.post", "label.pre", "label.post"], 
